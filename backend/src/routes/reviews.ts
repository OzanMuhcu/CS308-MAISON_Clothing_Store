...
// Returns every comment (pending, approved, rejected) with user + product context
// so the PM can see the full history and tell statuses apart at a glance.
// Optional ?status=pending|approved|rejected narrows the result if the UI wants
// to render a single tab.
